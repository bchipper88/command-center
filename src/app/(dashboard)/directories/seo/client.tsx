...
type Keyword = {
  id: string; site_id: string; keyword: string; volume: number | null; cpc: number | null;
...
export function DirectoriesSEOClient({ keywords }: { keywords: Keyword[] }) {
  ...
  const filtered = siteKeywords.filter(k => {
  ...
  const totalVolume = siteKeywords.reduce((s, k) => s + (k.volume || 0), 0)
  const totalClicks = siteKeywords.reduce((s, k) => s + (k.gsc_clicks || 0), 0)
  const rankedKws = siteKeywords.filter(k => k.gsc_position)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 font-mono">Keywords & rankings across directories</p>
      </div>

      {/* Site tabs */}
      <div className="flex gap-2">
        {sites.map((s) => {
          const count = keywords.filter(k => k.site_id === s.id).length
          return (
            <button
              key={s.id}
              onClick={() => { setActiveSite(s.id); setSearch(''); setDiffFilter('all') }}
